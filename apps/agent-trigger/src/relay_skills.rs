...
pub(super) fn build_worker_prompt(context: WorkerPromptContext<'_>) -> String {
    let browser_enabled = context
        .intent
        .required_capabilities
        .iter()
        .any(|capability| capability == AGENT_EXECUTION_CAPABILITY_BROWSER);
    let browser_context = if browser_enabled {
        "本轮已启用 browser capability，可以使用 Relay 托管的 chrome-devtools MCP 完成浏览器操作和验证。"
    } else {
        "本轮未加载浏览器运行时，以减少资源占用。不要寻找或绕过缺失的浏览器工具；只有实际工作确认必须进行 UI 交互、截图或浏览器 E2E 时，调用 agent.work_session capability_request，传入当前 Intent ID 和 capability=browser，然后立即结束本 Turn。Relay 会从同一项目会话继续并按需启用浏览器。"
    };
