...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        execution_requested := NEW.status = 'active';
    ELSE
        execution_requested := NEW.status = 'active' AND (
            NEW.status IS DISTINCT FROM OLD.status
            OR NEW.next_run_at < OLD.next_run_at
            OR (
                NEW.manual_run_requested_at IS NOT NULL
                AND NEW.manual_run_requested_at IS DISTINCT FROM OLD.manual_run_requested_at
            )
            OR (
                NEW.wake_requested_at IS NOT NULL
                AND NEW.wake_requested_at IS DISTINCT FROM OLD.wake_requested_at
            )
        );
    END IF;
    ...
    INSERT INTO realtime_events (
        ...
        jsonb_build_object(
            ...
            'manual_run_requested_at', NEW.manual_run_requested_at,
...
BEGIN
    PERFORM pg_notify(
        ...
        json_build_object(
            ...
            'execution_requested', CASE
                WHEN NEW.payload ? 'execution_requested'
                    THEN (NEW.payload->>'execution_requested')::boolean
                ELSE NULL
            END
        )::text
    );
